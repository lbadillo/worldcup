...
public interface MatchRepository extends JpaRepository<Match, Long> {
    ...
    List<Match> findByGroupAndStageWithResult(@Param("groupId") Long groupId,
                                              @Param("stageId") Long stageId);

    @Query(MatchQueries.FIND_BY_STAGE_WITH_RESULT)
    List<Match> findByStageWithResult(@Param("stageId") Long stageId);

    @Query(value = MatchQueries.FIND_BY_DATE_AND_USER, nativeQuery = true)
    List<UserBetIDTO> findByDateAndUser(@Param("startDate") Instant startDate,
